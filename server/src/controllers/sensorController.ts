...
// ✅ センサーデータを保存する
export const saveSensorData = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
        const { timestamp, temperature, humidity, motion, flame } = req.body;

        if (!timestamp || temperature === undefined || humidity === undefined || motion === undefined || flame === undefined) {
            res.status(400).json({ message: "❌ 必要なデータが不足しています" });
        }

        const formattedTimestamp = new Date(timestamp).toISOString();
        const newData = new SensorData({ timestamp: formattedTimestamp, temperature, humidity, motion, flame });

        await newData.save();
        console.log("✅ 新しいデータが保存されました:", newData);
        res.status(201).json({ message: "✅ データが正常に保存されました", data: newData });
    } catch (error) {
